...
namespace ECE141 {
  ...
  Token& Tokenizer::peek(int anOffset) {
    ...
    Token& Tokenizer::peekStart(int anOffset) {
        return tokenAt(anOffset);
    }
  ...
  bool Tokenizer::skipIf(Operators anOperator) {
    ...
    bool containsParenthesis(const std::string& str) { //-AJ
        return str.find(')') != std::string::npos;
    }

    char removeClosingParenthesisAtEnd(std::string& str) {
        if (!str.empty() && str.back() == ')' || str.back() == ',') {
            char removedChar = str.back();
            str.pop_back();
            return removedChar;
        }
        return '\0';
  ...
  StatusResult Tokenizer::tokenize() {
    ...
    while(!input.eof()) {
      ...
      if(isPunctuation(theChar)) {
        ...
        std::string aString=readUntil(theChar);
          if(containsParenthesis(aString)){//-AJ fix
              char correction=removeClosingParenthesisAtEnd(aString);
              //seperate string and )
              theToken.data=aString;
              theToken.keyword=Keywords::identifier_kw;
              tokens.push_back(theToken);

              Token theToken{TokenType::punctuation, Keywords::unknown_kw, Operators::unknown_op};
              theToken.data.push_back(correction);
              tokens.push_back(theToken);
          }
          else{
              theToken.data=aString;
        ...
        if(Keywords::unknown_kw!=theToken.keyword) {
          ...
          if(containsParenthesis(theString)){
              char correction=removeClosingParenthesisAtEnd(theString);
              //seperate string and )
